...
case $OSTYPE in
	darwin*)
		...
		# macOS does not provide memrchr, so we need to include our implementation
		MEMRCHR_OBJ='memrchr.o'
		;;
	msys | cygwin)
		echo -e "Windows detected for build host.\n"
		CPU_THREADS=$NUMBER_OF_PROCESSORS
		# Windows does not provide memrchr, so we need to include our implementation
		MEMRCHR_OBJ='memrchr.o'
		;;
	linux* | *)
		echo -e "Assuming OS = Linux for build host.\n"
		CPU_THREADS=$(nproc --all)
		# Linux provides memrchr, so we don't need our implementation
		MEMRCHR_OBJ=''
		;;
esac
